...
public class WishlistServiceImpl implements IWishlistService {
	...
	public ResponseEntity<WishlistResponse> AddWishlist(int id, int userId) {
		...
		Optional<Wishlist> wishlistObj = Optional.ofNullable(wishlistDao.getWishListByName(book.getBookName()));
		if (!wishlistObj.isPresent()) {
			...
			return ResponseEntity.status(HttpStatus.ALREADY_REPORTED)
					.body(new WishlistResponse(208, "Book Already Added to wishlist"));
		}
	}
	...
	public ResponseEntity<WishlistResponse> getWishlist(int userId) {
		...
		if (wishlists.isPresent()) {
			return ResponseEntity.status(HttpStatus.ACCEPTED)
					.body(new WishlistResponse(202, "total books in wishlist " + wishlists.get().size(), wishlists.get()));
		} else {
			return ResponseEntity.status(HttpStatus.ACCEPTED)
					.body(new WishlistResponse(202, "No Books Added to wishlist"));
		}
	}
	...
	public ResponseEntity<WishlistResponse> getUserWishlist(String token) {
		...
		if (wishlists.isPresent()) {
			return ResponseEntity.status(HttpStatus.ACCEPTED).body(
					new WishlistResponse(202, "total books in wishlist " + wishlists.get().size(), wishlists.get()));
		} else {
			return ResponseEntity.status(HttpStatus.ACCEPTED)
					.body(new WishlistResponse(202, "Not any Books Added to wishlist"));
		}
	}
	...
	public ResponseEntity<WishlistResponse> removeWishlist(int userId, int id) {
		...
		return ResponseEntity.status(HttpStatus.ACCEPTED)
				.body(new WishlistResponse(202, "Book removed from wishlist SuccessFully"));
	}

	/*********************************************************************
	 * To remove the book from wishlist by the user it will remove book from
	 * wishlist.
	 * 
